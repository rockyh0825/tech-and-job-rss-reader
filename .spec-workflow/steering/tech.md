...
- **Micrometer Tracing**(micrometer-tracing-bridge-otel + opentelemetry-exporter-otlp + datasource-micrometer-spring-boot): HTTP リクエストと JDBC クエリのスパンを生成し OTLP/HTTP で Tempo へ送信。SQL スパン(SQL 文つき・バインド値なし)がクエリ単位で出るのが要点。全量サンプリング
