...
- **既存テストスイート**: テストコードの検証内容(冪等性・集計・catch-up)は変更せず、実行基盤だけ Testcontainers に載せ替える
...
- 追加: `org.postgresql:postgresql`、`org.flywaydb:flyway-database-postgresql`(Flyway 10 系は DB モジュールが分離されている)、`org.springframework.boot:spring-boot-testcontainers`、`org.testcontainers:postgresql`、`org.testcontainers:junit-jupiter`
- 削除: `org.xerial:sqlite-jdbc`
...
spring-data-jdbc は PostgreSQL の Dialect を同梱するため、SPI 補完クラスと `META-INF/spring.factories` の登録を削除する。
...
- DB を使うテスト(リポジトリ・EmbeddedKafka 結合・MockMvc で ApplicationContext を立てるもの)は Testcontainers の PostgreSQL コンテナに接続する
- Spring Boot の `@ServiceConnection` + 共有コンテナ(static singleton)でコンテキスト間のコンテナ再利用を図り、テスト全体の起動回数を最小化する
...
- RssItemRepositoryTest: 検証内容(冪等 insert・集計・期間境界)は維持し、接続先を Testcontainers の PostgreSQL に変更。タイムスタンプが型として往復すること(保存 → 読み出しで `Instant` が一致)を確認するケースを追加
