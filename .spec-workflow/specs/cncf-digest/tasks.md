...
- [ ] 2. CncfProjects 辞書 + CncfProjectMatcher(domain)を実装(テスト込み)
...
- [ ] 3. CncfDigestSelectionPolicy + CncfDigest + CncfDigestPublisher(domain)を実装(テスト込み)
...
- [ ] 4. DiscordPoster を抽出(純リファクタ・単独コミット)
...
- [ ] 5. CncfDiscordWebhookClient(infrastructure)を実装(テスト込み)
...
- [ ] 6. BuildCncfDigestUseCase(application)を実装(テスト込み)
...
- [ ] 7. CncfDigestScheduler + 条件付き Bean 再編を実装(テスト込み)
